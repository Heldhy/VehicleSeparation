...
def extract_single_vehicles(base_img, unique_vehicles):
    ...
    img = mark_boundaries(base_img, markers, color=[0.0, 1.0, 0.0], mode="outer")
...
def merging_single_and_clustered_results(base_img, unique_markers, clustered_markers, nb_single_vehicles):
    ...
    img = mark_boundaries(base_img, merged_markers, color=[1.0, 0.0, 0.0], mode="outer")
...
def main():
    ...
    if (TERMINAL_MODE and len(sys.argv) < 3):
        ...
        unique_vehicles, clustered_vehicles, single_vehicle_mean_size = separate_vehicles_by_size(denoised_image,
                                                                                                  separation_size=160)
        unique_vehicles_results, unique_markers, nb_single_vehicles = extract_single_vehicles(img, unique_vehicles)
        clustered_vehicles_results, clustered_markers, nb_clustered_vehicles = extract_clustered_vehicles(img,
                                                                                                          clustered_vehicles,
                                                                                                          single_vehicle_mean_size)
        segmented_vehicles_results, markers, nb_vehicles = merging_single_and_clustered_results(background_image,
                                                                                                unique_markers,
                                                                                                clustered_markers,
                                                                                                nb_single_vehicles)
